...
# Winsound is Windows-only, use platform-independent alternative
...
def test_ac(test_no):
    if test_no > aktif_test:
        messagebox.showwarning("Uyarı", "Bu teste geçmek için önceki testi tamamlamalısınız.")
        return
    ...
    def soru_goster():
        soru_label.config(text=f"Soru {index+1}: {sorular[index][0]}")
        cevap.set(None)
        for i, secenek in enumerate(sorular[index][1]):
            radio_btns[i].config(text=secenek, value=chr(65+i))

    soru_label = tk.Label(pencere, text="", wraplength=600, font=("Arial", 12))
    soru_label.pack(pady=20)

    radio_btns = []
    for i in range(4):
        rb = tk.Radiobutton(pencere, text="", variable=cevap, value="", font=("Arial", 11))
        rb.pack(anchor="w")
        radio_btns.append(rb)

    onay_btn = tk.Button(pencere, text="Cevabı Onayla", command=sonraki_soru)
...
def sonuc_goster(test_no, dogru_sayisi):
    ...
    messagebox.showinfo("Test Sonucu", f"Doğru Sayısı: {dogru_sayisi}/10\nDurum: {durum}")
    if dogru_sayisi >= 7:
        tamamlanan_testler.add(test_no)
        aktif_test = test_no + 1
        kutular[test_no-1].config(text=f"TEST_{test_no}\n✅", bg="lightgreen")

# Test kutularını oluştur
kutular = []
def kutu_olustur(frame, test_no, row, col):
    btn = tk.Button(frame, text=f"TEST_{test_no}\n10 SORU VARDIR\nBAŞARI DURUMU\n(GEÇTİ-KALDI)",
                    bg="yellow", fg="black", width=20, height=5,
...
pozisyonlar = [(0, i) for i in range(4)] + [(1, i) for i in range(3)] + [(2, i) for i in range(3)]
...
#  & "C:\Users\USER\Islamı_Test_Soruları/.venv/Scripts/python.exe" islami_test_python_41.py


#  & "E:/İslami Test Soruları/.venv/Scripts/python.exe" islami_test_python_41.py   Pardusda

# python3 "/home/ismail/Documents/Python/gtk-dersi/data/İslami Test/Soruları/islami_test_python_41.py" pardusda

#  & "C:\Users\USER\Islamı_Test_Soruları/.venv/Scripts/python.exe" islami_test_python_41.py     windows da
...
'''
CEVAP ANAHTARI...ilk denemedir test çalışıyor...sonrakinde sertifika eklemek istiyorum .. 
