...
# ✅ 50 SORULUK TAM İSLAMİ BİLGİ TESTİ
test_sorular = [
...
def test_goster(sorular):
    print("\n" + "="*50)
    print("📚 İSLAMİ BİLGİ TESTİ - 50 SORU 📚")
    print("="*50 + "\n")
    
    dogru = 0
    for i, s in enumerate(sorular, 1):
        print(f"SORU {i}: {s['soru']}")
        ...
        while True:
            ...
            if cevap in "ABCD":
                break
            print("A, B, C veya D girin!")
        
        if cevap == s['dogru']:
            print("✅ DOĞRU!")
            dogru += 1
        else:
            print(f"❌ YANLIŞ! Doğru: {s['dogru']}")
        print("-"*40)
    
    yuzde = (dogru / 50) * 100
    print("\n" + "="*50)
    print(f"SONUÇ: {dogru}/50 (%{yuzde:.1f})")
    print("="*50)
    
    if yuzde >= 90: print("🏆 MÜKEMMEL!")
    elif yuzde >= 80: print("🥇 ÇOK İYİ!")
    elif yuzde >= 70: print("🥈 İYİ!")
    else: print("📚 ÇALIŞ!")

def main():
    while True:
        print("\n1. Tam Test (50 soru)")
        print("2. Rastgele 20 soru")
        print("3. Rastgele 10 soru")
        print("0. Çıkış")
        
        sec = input("Seçim: ")
        if sec == "1":
            test_goster(test_sorular)
        elif sec == "2":
            rastgele = random.sample(test_sorular, 20)
            test_goster(rastgele)
        elif sec == "3":
            rastgele = random.sample(test_sorular, 10)
            test_goster(rastgele)
        elif sec == "0":
...
# 50 soru var terminalde çalışıyor.....
...
#  & "C:\Users\USER\Islamı_Test_Soruları/.venv/Scripts/python.exe" islami_test_python_40.py


#  & "E:/İslami Test Soruları/.venv/Scripts/python.exe" islami_test_python_40.py   Pardusda

# python3 "/home/ismail/Documents/Python/gtk-dersi/data/İslami Test/Soruları/islami_test_python_40.py" pardusda

#  & "C:\Users\USER\Islamı_Test_Soruları/.venv/Scripts/python.exe" islami_test_python_40.py     windows da
